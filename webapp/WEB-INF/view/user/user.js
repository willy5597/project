...
const loadHTML = async(url, containerId) => {
	const fullUrl = 'http://localhost:8080/LeeSofa' + url;
	try {
 ...
 *********************************************************************************************************/
// 渲染 User 資料配置
const renderUser = ({id, name, email, password}) => `
	<tr>
...
const addUser = async(formData) => {
	...
	fetchAndRenderData('/user', 'user-list-body', renderUser);
	// 清除表單
	clearForm();
};
...
const getUserData = async(id) => {
	const fullUrl = `http://localhost:8080/user/${id}`;
	try {
